...
Run with Blender:
blender --background --python database/scan_shipwreck_fbx.py -- \
  --models-dir "3d modeling/shipwreck/original_models" \
  --metadata database/shipwreck_size_orientation_metadata.csv \
  --output-csv database/generated/shipwreck_size_orientation.csv \
  --output-json database/generated/shipwreck_size_orientation.json
"""
...
def measure(path: Path) -> dict:
    ...
    # Positive longest axis is treated as provisional bow direction.
    # The width sign is selected so that length x width = up.
...
def main() -> int:
    ...
    for filename in all_names:
        ...
        try:
            ...
            if scale:
                row["scaled_width_from_model_m"] = rounded(
                    number(str(row["model_width_units"])) * scale
                )
                row["scaled_height_from_model_m"] = rounded(
                    number(str(row["model_height_units"])) * scale
                )
        ...
        json.dumps(
            {
                ...
                "coordinate_convention": {
                    ...
                    "left_width": STANDARD_LEFT,
                    "up": STANDARD_UP,
                },
                "orientation_warning": (
                    "Bounding-box axes are measured automatically, but bow and up signs "
                    "must be confirmed visually before simulation."
                ),
